...
with open(os.path.join(base_dir, "bg_base64.txt"), "r") as f:
    bg_b64 = f.read().strip()

html_content = f"""  const splashHtml = `
    <!DOCTYPE html>
    <html>
    <head>
...
pattern = re.compile(r"  const splashHtml = `.*?\n  `;", re.DOTALL)
new_content = pattern.sub(html_content, content)

with open(main_cjs_path, "w", encoding="utf-8") as f:
    f.write(new_content)

print("Updated main.cjs with new splash screen successfully.")
